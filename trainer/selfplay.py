...
def run_selfplay(init_ckpt: str, net_cfg, n_workers: int, total_steps: int,
    ...
    if resume and state_path.exists():
        ...
        if st.get("net_config") == asdict(net_cfg):
            champion.load_state_dict(st["champion"])
            train_net.load_state_dict(st["train_net"])
            ...
            print(f"[selfplay] RESUMED: step={steps} games={games} promotions={promotions} "
                  f"est.Elo~{elo_est:.0f} sims={cur_sims}")
        else:
            print("[selfplay] state net_config mismatch — starting fresh from init-from")
